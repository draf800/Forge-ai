...
export default function Builder() {
  ...
  async function handleSave() {
    ...
    const res = await fetch("/api/projects", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        name: turns[0]?.text?.slice(0, 60) || "Untitled",
        prompt: turns.map((t) => t.text).join(" / "),
        files: site.files,
      }),
    });
    ...
    <main className="flex h-screen flex-col bg-forge-bg text-forge-paper">
      <header className="flex items-center justify-between border-b border-forge-line px-6 py-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {hasBackend && (
            <button
              onClick={handleActivateBackend}
              className="rounded-sm border border-forge-line px-3 py-1.5 text-xs hover:border-forge-brass"
            >
              Activate backend
            </button>
          )}
          ...
          <button
            ...
            disabled={!site}
            className="rounded-sm border border-forge-line px-3 py-1.5 text-xs hover:border-forge-brass disabled:opacity-40"
          >
            Download .zip
          </button>
          <button
